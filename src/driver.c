...
int driver_find(uint8_t *ip, char *if_name, uint8_t *mask) {
  ...
  if (pcap_findalldevs(&alldevs, pcap_errbuf) == -1) {
    Err("Error in pcap_findalldevs: %s", pcap_errbuf);
    return -1;
  }
  ...
  for (d = alldevs; d; d = d->next, if_num++)
    for (a = d->addresses; a; a = a->next)
      if (a->addr && a->addr->sa_family == AF_INET) {
        ...
        if (match[if_num] <
            ip_prefix_match((uint8_t *) &mask_all, (uint8_t *) &((struct sockaddr_in *) (a->netmask))->sin_addr.s_addr))
          match[if_num] = 0;
      }
  if (if_num == 0) {
    Err("Error, no interface found.");
    return -1;
  }
  uint8_t max_match = 0;
  size_t max_if = 0;
  ...
  if (max_match == 32) {
    Err("Error, interface %s have the same ip %s with me.", d->name, iptos(net_if_ip));
    return -1;
  }
  for (a = d->addresses; a; a = a->next)
    if (a->addr && a->addr->sa_family == AF_INET)
...
int driver_open() {
  ...
  if (driver_find(net_if_ip, if_name, (uint8_t *) &mask) < 0) {
    Err("Error in driver find.");
    return -1;
  }
  Log("Using interface %s, my IP is %s, my MAC is %s", if_name, iptos(net_if_ip), mactos(net_if_mac));

  // 混杂模式打开网卡
  if ((pcap = pcap_open_live(if_name, 65536, 1, 10, pcap_errbuf)) == NULL) {
    Err("Error in pcap_open_live.\n%s.", pcap_errbuf);
    return -1;
  }
  // 设置非阻塞模式
  if (pcap_setnonblock(pcap, 1, pcap_errbuf) < 0) {
    Err("Error in pcap_setnonblock. %s.", pcap_errbuf);
    return -1;
  }
  ...
  if (pcap_compile(pcap, &fp, filter_exp, 0, mask) < 0) {
    Err("Error in pcap_compile: %s.", pcap_geterr(pcap));
    return -1;
  }
  if (pcap_setfilter(pcap, &fp) < 0) {
    Err("Error in pcap_setfilter: %s.", pcap_geterr(pcap));
    return -1;
  }
  return 0;
}
...
int driver_send(buf_t *buf) {
  if (pcap_sendpacket(pcap, buf->data, buf->len) == -1) {
    Err("Error in driver_send: %s.", pcap_geterr(pcap));
    return -1;
  }
